...
\section{Conclusions}
The experiments carried out with the \textit{Transfer Learning} procedure have given excellent results. The technique is certainly a useful and valid approach to build excellent models quickly and with low computational capacity, even if the risk of overfitting is always present and one must pay close attention to the adaptability with one's own dataset, one's own domain and its own classes, based on the chosen model.
